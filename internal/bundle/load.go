...
type statement struct {
	file  string
	index int
	// subIndex distinguishes multiple statements sharing the same
	// (file, index): always 0 for a real, source-parsed statement; >0
	// only for a statement synthesized by the FK-cycle-breaking pass
	// (see cyclebreak.go), which shares its origin statement's (file,
	// index) and sorts after it via SubIndex.
	subIndex int
	ps       parse.Statement
	// synthesized is true for an ALTER TABLE statement generated by the
	// FK-cycle-breaking pass rather than parsed directly from source; it
	// controls the leading comment emit prepends (see emit.go).
	synthesized bool
	// synthesizedFrom names the source file the synthesized statement's
	// FK clause was extracted from, for that leading comment.
...
func (l Loaded) graphNodes() []graph.Node {
	...
	for i, s := range l.Stmts {
		name := s.ps.Name
		if s.synthesized {
			// A synthesized ALTER TABLE (see cyclebreak.go) augments an
			// existing table rather than defining a new object; leaving
			// Name set would collide with — and, since graph.Sort's
			// byName resolution keeps the last writer, silently shadow —
			// the table's own CREATE TABLE node for edge resolution
			// purposes. It still participates via Deps (on both the
			// altered table and the FK target), just never as a resolve
			// target itself.
			name = ""
		}
		nodes[i] = graph.Node{
...
func keySub(file string, index, subIndex int) string {
	return fmt.Sprintf("%s\x00%d\x00%d", file, index, subIndex)
}
...
func CheckDiagnostics(root string, dialect Dialect, readFile func(path string) ([]byte, error)) ([]diag.Diagnostic, error) {
	...
	loaded, sortErr := breakFKCycles(loaded, dialect)
	if sortErr == nil {
		_, sortErr = graph.Sort(loaded.graphNodes())
	}

	if sortErr != nil {
		var cycleErr *graph.CycleError
		if errors.As(sortErr, &cycleErr) && len(cycleErr.Nodes) > 0 {
